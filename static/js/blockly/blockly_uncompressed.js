...
Blockly.BlockSvg.prototype.initSvg = function() {
    ...
    goog.isFunction(this.onchange) && !this.eventsInit_ && Blockly.bindEvent_(this.workspace.getCanvas(),
...
Blockly.createDom_ = function(a) {
    ...
    if (0 < Blockly.gridOptions.length) {
        c = Math.floor(Blockly.gridOptions.spacing / 2) + .5;
    ...
    Blockly.readOnly || (Blockly.hasCategories ? Blockly.mainWorkspace.toolbox_ = new Blockly.Toolbox(b, a) : Blockly.languageTree && Blockly.mainWorkspace.addFlyout(), Blockly.hasScrollbars || Blockly.addChangeListener(function() {
        if (0 == Blockly.dragMode_) {
            ...
            if (a.contentTop < c || a.contentTop + a.contentHeight >
                a.viewHeight + c || a.contentLeft < (Blockly.RTL ? a.viewLeft : b) || a.contentLeft + a.contentWidth > (Blockly.RTL ? a.viewWidth : a.viewWidth + b))
...
Blockly.init_ = function() {
    ...
    Blockly.bindEvent_(Blockly.svg, "touchstart", null, function(a) {
        Blockly.longStart_(a, null)
    });
    Blockly.documentEventsBound_ || (Blockly.bindEvent_(window, "resize", document, Blockly.svgResize), Blockly.bindEvent_(document, "keydown", null, Blockly.onKeyDown_), Blockly.bindEvent_(document,
        "touchend", null, Blockly.longStop_), Blockly.bindEvent_(document, "touchcancel", null, Blockly.longStop_), document.addEventListener("mouseup", Blockly.onMouseUp_, !1), goog.userAgent.IPAD && Blockly.bindEvent_(window, "orientationchange", document, function() {
        Blockly.fireUiEvent(window, "resize")
    ...
    if (Blockly.hasSounds) {
        ...
        b.push(Blockly.bindEvent_(document, "touchstart",
            null, a))
    }
};
...
Blockly.onMouseMove_ = function(a) {
    if (Blockly.mainWorkspace.dragMode) {
        Blockly.removeAllRanges();
        var b = a.clientX - Blockly.mainWorkspace.startDragMouseX,
            c = a.clientY - Blockly.mainWorkspace.startDragMouseY,
            d = Blockly.mainWorkspace.startDragMetrics,
            e = Blockly.mainWorkspace.startScrollX + b,
            f = Blockly.mainWorkspace.startScrollY + c,
            e = Math.min(e, -d.contentLeft),
            f = Math.min(f, -d.contentTop),
            e = Math.max(e, d.viewWidth - d.contentLeft - d.contentWidth),
            f = Math.max(f, d.viewHeight - d.contentTop - d.contentHeight);
        Blockly.mainWorkspace.scrollbar.set(-e -
            d.contentLeft, -f - d.contentTop);
        Math.sqrt(Math.pow(b, 2) + Math.pow(c, 2)) > Blockly.DRAG_RADIUS && Blockly.longStop_();
        a.stopPropagation()
    }
};
...
Blockly.longStart_ = function(a, b) {
    ...
    Blockly.longPid_ = setTimeout(function() {
        a.button = 2;
        if (b) b.onMouseDown_(a);
        else Blockly.onMouseDown_(a)
    }, Blockly.LONGPRESS)
};
...
Blockly.getMainWorkspaceMetrics_ = function() {
    ...
    if (Blockly.mainWorkspace.scrollbar) var f = Blockly.RTL ? 0 : Blockly.Scrollbar.scrollbarThickness,
        g = Math.min(d.x - b / 2, d.x + d.width - b - (Blockly.RTL ? Blockly.Scrollbar.scrollbarThickness : 0) + 5),
