  ...
  def sync_with_server(self):
    ...
    for p_name in self._repository.keys():
      ...
      if p.uid is None or progs_r.get(p.uid) == None:
        logging.info("saving: " + str(p.uid) + " remote: " + str(progs_r.get(p.uid)))
        ...
        if p_r and int(p_r.get("version")) < int(p.version):
          logging.info("updating: " + p.name + " version: " + str(p_r.get("version")) + " version: " + str(p.version))
          retval = api.CoderBotServerAPI.program_save(p.uid, p.name, p.as_json())
          self.save(p)
    ...
    for p_r in progs_r.values(): 
      if progs_l.get(p_r.get("uid")) is None or p_r.get("version") > progs_l.get(p_r.get("uid")).version:
